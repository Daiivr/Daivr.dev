...
// === Discord-linked storage wrapper for The Cube ===
const theCubeUrlParams = new URLSearchParams(window.location.search);
const theCubeDiscordId = theCubeUrlParams.get('discordId');
const THE_CUBE_HAS_DISCORD = !!theCubeDiscordId;
const theCubeMemoryStore = {};

function theCubeLsKey(key) {
  return THE_CUBE_HAS_DISCORD ? `discord_${theCubeDiscordId}_${key}` : key;
}

function theCubeLsSet(key, value) {
  if (!THE_CUBE_HAS_DISCORD) {
    theCubeMemoryStore[key] = String(value);
    return;
  }
  try {
    window.localStorage.setItem(theCubeLsKey(key), String(value));
  } catch (e) {
    console.warn('The Cube: localStorage set failed', e);
  }
}

function theCubeLsGet(key) {
  if (!THE_CUBE_HAS_DISCORD) {
    return Object.prototype.hasOwnProperty.call(theCubeMemoryStore, key)
      ? theCubeMemoryStore[key]
      : null;
  }
  try {
    return window.localStorage.getItem(theCubeLsKey(key));
  } catch (e) {
    console.warn('The Cube: localStorage get failed', e);
    return null;
  }
}

function theCubeLsRemove(key) {
  if (!THE_CUBE_HAS_DISCORD) {
    delete theCubeMemoryStore[key];
    return;
  }
  try {
    window.localStorage.removeItem(theCubeLsKey(key));
  } catch (e) {
    console.warn('The Cube: localStorage remove failed', e);
  }
}
...
class Timer extends Animation {
  ...
  update() {
    ...
    if ( this.converted != old ) {
      ...
      theCubeLsSet( 'theCube_time', this.deltaTime );
      this.setText();
...
class Storage {
  ...
  constructor( game ) {
    ...
    const userVersion = theCubeLsGet( 'theCube_version' );
    ...
    if ( ! userVersion || userVersion !== window.gameVersion ) {
      ...
      this.migrateScores();
      theCubeLsSet( 'theCube_version', window.gameVersion );
  ...
  loadGame() {
    ...
    try {
      ...
      const gameInProgress = theCubeLsGet( 'theCube_playing' ) === 'true';

      if ( ! gameInProgress ) throw new Error();

      const gameCubeData = JSON.parse( theCubeLsGet( 'theCube_savedState' ) );
      const gameTime = parseInt( theCubeLsGet( 'theCube_time' ) );
  ...
  saveGame() {
    ...
    theCubeLsSet( 'theCube_playing', gameInProgress );
    theCubeLsSet( 'theCube_savedState', JSON.stringify( gameCubeData ) );
    theCubeLsSet( 'theCube_time', gameTime );

  }

  clearGame() {

    theCubeLsRemove( 'theCube_playing' );
    theCubeLsRemove( 'theCube_savedState' );
    theCubeLsRemove( 'theCube_time' );
  ...
  loadScores() {
    ...
    try {
      ...
      const scoresData = JSON.parse( theCubeLsGet( 'theCube_scores' ) );
  ...
  saveScores() {
    ...
    theCubeLsSet( 'theCube_scores', JSON.stringify( scoresData ) );

  }

  clearScores() {

    theCubeLsRemove( 'theCube_scores' );
  ...
  migrateScores() {
    ...
    try {
      ...
      const scoresData = JSON.parse( theCubeLsGet( 'theCube_scoresData' ) );
      const scoresBest = parseInt( theCubeLsGet( 'theCube_scoresBest' ) );
      const scoresWorst = parseInt( theCubeLsGet( 'theCube_scoresWorst' ) );
      const scoresSolves = parseInt( theCubeLsGet( 'theCube_scoresSolves' ) );
      ...
      theCubeLsRemove( 'theCube_scoresData' );
      theCubeLsRemove( 'theCube_scoresBest' );
      theCubeLsRemove( 'theCube_scoresWorst' );
      theCubeLsRemove( 'theCube_scoresSolves' );
  ...
  loadPreferences() {
    ...
    try {
      ...
      const preferences = JSON.parse( theCubeLsGet( 'theCube_preferences' ) );
  ...
  savePreferences() {
    ...
    theCubeLsSet( 'theCube_preferences', JSON.stringify( preferences ) );

  }

  clearPreferences() {

    theCubeLsRemove( 'theCube_preferences' );
